...
def calc_loss_batch(input_batch, target_batch, model, device):
    ...
    # 【方案一：直接使用 HF 内部 Loss 计算】
    # HF 模型如果传入 labels，会自动计算 CrossEntropy 并忽略 -100
    outputs = model(input_batch, labels=target_batch)
    loss = outputs.loss

    if torch.isnan(loss):
        # 调试：检查是否因为 target_batch 全是 -100
        if (target_batch == -100).all():
            return torch.tensor(0.0, device=device, requires_grad=True)

        # 强制切换到 CPU 看看是否是 MPS 的硬件 bug
        logits = outputs.logits.to("cpu")
        targets_cpu = target_batch.to("cpu")
